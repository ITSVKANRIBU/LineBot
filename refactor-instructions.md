...
**承認済みの挙動変更は第 10 章に列挙した 10 件だけ**（Q1、Q2、Q3、Q4、Q5、Q6、Q7、Q10、Q11、Q12）。それ以外の挙動変更は禁止。Q8 は「現状を許容」、Q9 は「条件付きで DI 化を実施」、Q13 は「API の村なし応答はテキストのまま」。

---

## 1. Objective

- 外部から観測できる振る舞い（LINE への応答文、HTTP API の応答、配役規則、村番号の体系、上限値）を、第 10 章で承認された変更を除いて**変えずに**、Bot 本体の重複・責務混在・到達不能コード・脆い契約を減らす。
- 「LINE 経由」と「`/callapi` 経由」の処理を **LINE を正として共通化**する（Q11〜Q13）。入力の解釈（数値境界、コマンド表、trim）は 1 組だけになり、経路ごとに残るのは「入力の取り出し方」「返し方（返信 API か JSON か）」「対象の村がないときの応答（LINE は確認テンプレート、API はテキスト）」の 3 点だけにする（`docs/overview.md` 設計原則 4 の徹底）。
...
2. 外部フォーム → `/callapi` → `MainController` が同様に解釈（現状は境界値と `@` コマンドの扱いが異なる。Phase 4 で LINE に統一）→ 同じ `VillageService` → `List<Message>` を JSON で返す。`null` は `村が作成されていません` へ変換。
...
### 3.1 数値の解釈（LINE を正として統一する。Q1、Q11）

**現状**は経路で異なる。

| 入力 | LINE（現状 = 統一後の正） | `/callapi`（現状） |
| --- | --- | --- |
| 10000 以上 | 特殊村へ参加 | 特殊村へ参加 |
| 1000〜9999 | 通常村へ参加 | 通常村へ参加 |
| 101〜999 | 村番号扱い（該当なし → 村なし応答） | 人数設定 |
| 100 以下 | 人数設定 | 人数設定 |
| 数値の前後の空白 | `trim()` してから判定 | trim せず → お題文字列 |

**統一後**（Phase 4 完了時）は、両経路とも LINE 列のとおり。`/callapi` からも人数は 100 までになり、101〜999 は村番号として扱われて村なし応答（`村が作成されていません`）になる。外部フォームが 101 以上の人数を送る想定はないことをオーナーが確認済み。Phase 3b-4 の trim 統一は、この統一の先行分。
...
- `@逆村` `＠逆村`、`@わーわーず` `＠わーわーず`、`@取得` `＠取得`、`@配布` `＠配布`、`@特殊` `＠特殊`。**現状は LINE のみ**で `/callapi` ではお題文字列になるが、**Q12 で `/callapi` でも同じに解釈する**ことが承認済み（Phase 4）。`@取得` の応答はポストバックボタン付きテンプレートのままでよい（API 側にポストバックの入口はないが、「確定」ボタン相当の候補文字列は JSON から読める）。
...
- `/callapi`: `message` か `userId` が空 → 400（本文なし）。村なし → 200 で `村が作成されていません`（**Q13: 共通化後もこのテキスト応答を維持する**。LINE の確認テンプレートに揃えない。外部フォームがテンプレートを表示できるか不明なため）。内部エラー → 500 `{"error":"内部エラーが発生しました。"}`。CORS 有効。
...
11. `docs/` は契約。コードを docs に合わせるのが原則で、docs を書き換えるのは第 10 章で承認された範囲（Q3 の辞書境界、Q6 のビルド設定、Q10 の CLI 削除、Q1 の trim 統一の追記、Q11・Q12 の経路統一）だけ。docs の更新は対応するコード変更と同じコミットで行う。
...
- なぜ負債か: コマンド追加時に片方だけ直すと経路間で規則が分岐し、`docs/architecture.md` が指摘するとおり誰も気付けない。境界値とコマンド表の差は、経路ごとに別々に実装された結果で、意図した仕様ではない（Q11・Q12 で LINE に揃えることが承認済み）。
- 影響範囲: 両アダプタ、`docs/interfaces.md`・`architecture.md`・`roadmap.md`・`game-spec.md`、`sample-spring-boot-echo/README.md`。
- 変更リスク: 中。`/callapi` の挙動が 2 点変わる（101〜999 が村番号扱いになる、`@` コマンドが解釈される）。文字列一致の順序と `trim()` の扱いは LINE の現状を正とする。
- 改善案: 経路を知らない共通入口（例 `spring/game` の `TextCommandHandler`）を 1 つ置く。契約は「`userId` と入力文字列を受け取り、返信すべき `List<Message>` を返す。対象の村がなければ `null`」。中身は現在の `EchoApplication.replyMessage` の分岐をそのまま移す（`trim()` 後に数値判定、`> 9999` 特殊村、`> 100` 通常村、それ以外は人数。文字列は `お題`/`題`/`神`/`ランダム`/`@` コマンド 5 種の順に判定し、残りはお題として**未 trim のまま** `setOdai` へ）。`@取得` は現在 `replyToken` で直接返信しているが、共通入口では `List<Message>` を返す形にする。LINE ハンドラは `userId` の null 拒否 → 共通入口 → `null` なら確認テンプレート、それ以外は返信。`MainController` は空パラメータの 400 → 共通入口 → `null` なら `村が作成されていません` テキスト（Q13）、それ以外は JSON。境界値は 1 組の定数だけ持つ（D15）。
- 検証: `MainControllerTest` 全件（既存は人数 2〜5 しか使わないので不変）+ Phase 2 の LINE テキスト経路テスト + 新規に API 側で「`@逆村` が動く」「`101` が村なし応答になる」「`@取得` がテンプレートを返す」を追加。両経路で同じ入力に同じ `List<Message>` が返ることを、代表的な入力（村作成・お題・人数・参加・満員・`@逆村`・未知の文字列）で突き合わせるテストを 1 本置く。
- 判定: **実装可**（Phase 4。Q11・Q12・Q13 承認済み）。docs の更新箇所は Phase 4 の手順に列挙。
...
- 改善案: 意味のある名前の定数へ（例: `MAX_SIZE_INPUT = 100`、`MIN_VILLAGE_NUM = 1000`、`MIN_SPECIAL_VILLAGE_NUM = 10000`）。Phase 3 では両経路の値をそれぞれ現状のまま定数化し（API 側は 999）、Phase 4 の共通化で API 側の 999 を捨てて 1 組にする（Q11）。
- 判定: **実装可**（Phase 3〜4）。
...
4. 注意: Phase 2 は**現状の挙動**を固定する。第 10 章で承認済みの挙動変更に該当する箇所（`/callapi` の空白付き数値、`/callapi` の 101〜999、`/callapi` での `@` コマンド、Werewords の欠け表記が村人の場合、難易度 1、カタログの不正要素）は Phase 2 では**テストを書かず**、Phase 3b・Phase 4・Phase 5 で変更と同時にテストを追加する。LINE 側の現状はそのまま固定してよい（LINE が正なので変わらない）。
...
3. D1: 共通入口を導入し、`EchoApplication.replyMessage` と `MainController.messageController/nonNumberMessage` をそれに載せ替える（Q11・Q12・Q13）。LINE 側の挙動は不変。`/callapi` 側は 101〜999 が村なし応答になり、`@` コマンドが解釈されるようになる。村なし応答のテキストは維持。**2 コミットに分ける**: (3a) 共通入口を作り LINE ハンドラだけ載せ替える（挙動不変。Phase 2 のテストで確認）、(3b) `MainController` を載せ替え、API の新挙動のテストと docs 更新を同じコミットに含める。docs の更新箇所:
   - `docs/interfaces.md`「数値の解釈」: 表を 1 列にし、「この不一致は意図的な統一の結果ではなく…」以下の段落を削除。「この API が持たない機能」から「`@` で始まるコマンドを解釈しません」の項を削除し、`@取得` の応答にはポストバックボタンが含まれる旨を 1 文残す。「LINE から設定できる参加人数は 100 人まで…一方 `/callapi` からは 999 人まで」を「参加人数は両経路とも 100 人まで」に。
   - `docs/architecture.md`「入力アダプタの責務を『解釈だけ』に閉じる理由」の「実際に、両経路で異なるのは数値の境界値と対応コマンドだけです」を「両経路で異なるのは、対象の村がないときの応答（LINE は確認テンプレート、API はテキスト）だけです」に。「既知の制約」の「経路ごとの数値境界の不一致」の節を削除。
   - `docs/roadmap.md`「課題として認識しているが、計画していないもの」の表から「経路ごとの数値境界の不一致」の行を削除。
   - `docs/game-spec.md`「参加人数」の「LINE からは 100 人までしか指定できません。101 以上は村番号として解釈されるためです」を「100 人までです。101 以上は村番号として解釈されます」に。
   - `sample-spring-boot-echo/README.md` の「LINEと`/callapi`で村番号の判定基準が異なる点に注意してください」の段落を削除し、共通入口のクラスを構成に追記。
...
| Q11 | LINE と `/callapi` で数値境界が異なる（API は 101〜999 を人数として受け付ける） | **LINE に揃える**。API からも人数は 100 まで。外部フォームが 101 以上を送る想定はない | Phase 4-3（D1、D15）、docs 更新 |
| Q12 | `/callapi` は `@` コマンドを解釈せずお題文字列にする | **LINE に揃える**。API でも `@逆村` `@わーわーず` `@取得` `@配布` `@特殊` を解釈する | Phase 4-3（D1）、docs 更新 |
| Q13 | 対象の村がないときの応答（LINE は確認テンプレート、API は `村が作成されていません` テキスト） | **API はテキストのまま**。外部フォームがテンプレートを表示できるか不明なため、ここだけ経路差を残す | 3.7、D1 |

Q2 と Q7 の影響範囲は「役職メッセージに添える画像の選択」だけで、配役・お題・webhook 応答には関わらない。取り込みは 5 分ごとの専用スレッドで動き、webhook の処理スレッドとは別。

Q11〜Q13 は 2026-09-07 に追加で回答を得た。共通化後に経路ごとに残る差は「入力の取り出し方」「返し方」「村なし応答」の 3 点だけで、ゲーム規則・数値境界・コマンド表・trim はすべて LINE の現状と同一になる。
...
- `/callapi` へのポストバック・スタンプ相当の入口の追加。LINE の確認テンプレートを API でも返すこと（Q13 で見送り）。
- 状態の永続化、複数プロセス対応。
